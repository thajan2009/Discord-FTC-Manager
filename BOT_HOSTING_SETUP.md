...
## 5. Command map (final)
- Hybrid (both): `!bal|!balance|!finance` ↔ `/balance [team]`, `!inv|!inventory` ↔ `/inventory [team]`, `!ping`↔`/ping`, `!help`↔`/help`, `!status`↔`/status`, `!manage`↔`/manage` (posts `{PUBLIC_WEB_URL}/guild/<server id>`), plus `!findel <exact name>`, `!invdel <exact name>`.
